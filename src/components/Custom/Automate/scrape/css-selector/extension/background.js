...
import '@/assets/3rdparty/selector-gadget/selectorgadget_combined.min.js'

const Scripts = []
// 'assets/3rdparty/selector-gadget/selectorgadget_combined.min.js',
// 'js/content.js'
// ]
...
function inject (tab) {
  ...
  Scripts.forEach(f => browser.tabs.executeScript(tab.id, {file: f, runAt: 'document_end'}).then(r => console.log(f, r)))
...
function addHeaders (e) {
  ...
  if (h && h.value.includes('script-src')) {
    h.value = h.value.replace(/script-src [^;]*/, v)
  } else if (h) {
    h.value += ';' + v
  } else {
    e.responseHeaders.push({name: k, value: v})
  }
...
// { webRequest, browserAction, storage, tab, runtime}
