...
const BingAPI = options => {
  let xhr = new XMLHttpRequest(), data, config, result, id
      ...
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          ...
          data.images.forEach(image => {
            result.push(new Image(id++, `https://cn.bing.com${image.url}`, image.enddate, image.copyright))
          })
          // 根据日期 顺排序
          ...
          reject(new Error('internet error / [bing]'))
        }
      }
    }
  })
