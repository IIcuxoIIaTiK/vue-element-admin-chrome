...
export default {
  ...
  computed: {
    ...mapGetters(['application', 'bookmark']),
    time () {
      setInterval(() => {
        this.date = new Date()
      }, 1000)
      let hours = this.date.getHours(), mins = this.date.getMinutes()
  ...
  methods: {
    handler (name) {
      for (let type in this.dialog) {
        type === name ? this.$set(this.dialog, type, !this.dialog[type]) : this.$set(this.dialog, type, false)
      }
    },
    closeDialog (name) {
