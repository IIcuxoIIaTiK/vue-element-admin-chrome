<template>
  <div class="item">
    <i @click="bookmarkHandler" class="material-icons">{{option.icon}}</i>
     <transition name="top">
        <div class="bookmark dialog" v-if="visible">
          <div class="search">
            <input type="text" autofocus v-model="search_text" placeholder="search..."/>
            <span class="button" @click="close">Dismiss</span>
          </div>
          <div class="inner">
            <div class="category">
              <p v-for="category in categories" :key="category.id" @click="changeCategory(category.id)">
                {{ category.title }}
              </p>
            </div>
            <div class="item">
              <a v-for="(bookmark,idx) in current" :key="idx" :href="bookmark.url" target="_blank">
